<template>
    <div id="workshopsGrid">
        <div class="workshopsGridSection center">
            <p class="bellotaBold" id="categoryTitle">Compras<br>realizadas</p>
        ...
        <div class="workshopsGridSection" id="wrokshopsGrid">
            <div class="wrokshopsGridSection" v-for="(product, index) in products" :key="product.productDetails._id" @click="goToProduct(product.productDetails._id)">
                <div class="wrokshopsGridSectionDiv">
                    <img class="workshopImg" :src="product.productDetails.photos[0]" alt="Workshop Image">
                </div>
                <div class="wrokshopsGridSectionDivB">
                    <p class="wrokshopsGridSectionText bellotaBold">{{ product.productDetails.name }}</p>
                    <p class="wrokshopsGridSectionPrice bellotaBold">${{ product.productDetails.price }}</p>
                    <p class="wrokshopsGridSectionPrice bellotaBold">{{ product.productDetails.date }}</p>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        fetchFavorites() {
            ...
            .then(data => {
                this.products = data.data.flatMap(order => order.products);
            })
            .catch(error => {
...
<style scoped>
    #workshopsGrid {
        display: grid;
        grid-template-rows: 80px 20px calc(100% - 140px);
        width: 100vw;
        height: 100vh;
    }
    ...
    .wrokshopsGridSection {
        background-color: var(--color-B2-2);
        cursor: pointer;
        overflow: hidden;
        border-radius: 10px;
        position: relative;
